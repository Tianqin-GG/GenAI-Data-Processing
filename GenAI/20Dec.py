...
def save_website_url(company_names,news_urls):
    ...
    for company_name, news_url in zip(company_names, news_urls):
        print(company_name,news_url)
            # 假设 check_company_link 函数返回一个布尔值，表示是否找到匹配的链接
        if get_corporate_website(news_url, company_name):
            results.append({'Company Name': company_name, 'AURL': news_url})
        else:
    ...
    df.to_csv('result_url.csv', mode='a', header=False, index=False)  
...
save_website_url(data1['NAME'],data1['NEWS'])